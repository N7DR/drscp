// $Id: string_functions.cpp 3 2023-01-09 23:36:32Z n7dr $
...
{ string rv        { };
  ...
  while ((posn = s.find(old_char, last_posn)) != string::npos)
  { rv += (s.substr(last_posn, posn - last_posn) + new_char);
    last_posn = posn + 1;
  }
...
{ const auto cit { FIND_IF(vec, [ss] (const std::string& str) { return contains(str, ss); } ) };
  ...
  return distance(vec.cbegin(), cit);
}
...
{ optional<string> rv;
  ...
  { if (unsigned long posn { str.find(separator, start_posn) }; posn == string::npos) // no more separators, and we haven't found this field
