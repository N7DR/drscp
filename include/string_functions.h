...
/*! \brief              Split a string into components
    ...
    \return             vector containing the separate components
*/
//std::vector<std::string_view> split_string_sv(const std::string& cs, const std::string& separator);
...
inline std::vector<std::string_view> to_lines_sv(const std::string& cs, const char eol_marker = EOL_CHAR)
  { return split_string_sv(cs, eol_marker); }
...
*/
inline std::string force_end(const std::string& str, const std::string& required_end)
  { return ( ends_with(str, required_end) ? str : (str + required_end) ); }
...
{ std::string v_str { to_string(std::forward<T>(v)) };
  ...
  { if (posn_dp == v_str.length() - 1)  // ends in a dp
      return substring(v_str, 0, v_str.length() - 1);
    
    v_str = remove_trailing(v_str, '0');
